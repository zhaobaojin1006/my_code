...
/*转发文件线程函数*/
void server_filedata_thid (struct file_thid *file_thid_temp)
{
    /*设置线程自动释放资源*/
    ...
    struct online_list  *online_list_ptemp = online_list_PHEAD ;
    while(online_list_ptemp != NULL) {
        if(strcmp (online_list_ptemp -> user_ID, file_thid_temp ->from_ID) == 0) {
            break ;
        }
        online_list_ptemp = online_list_ptemp -> next ;
    }
    link_list_ptemp = online_list_ptemp -> link_list_head ;
    online_list_ptemp -> link_list_head = NULL ;
    if(online_list_ptemp == NULL) {
        printf ("源用户不在线\n") ;
        pthread_exit (0) ;
    }
    ...
    /*向源用户发送FILEDATA确认信息包*/
    struct cmd *cmd_temp = NULL ;
    cmd_temp = server_bag (file_thid_temp -> from_ID, file_thid_temp -> to_ID, file_thid_temp -> filename, FILEDATA) ;
    if(send (link_list_ptemp -> sock_fd, cmd_temp, sizeof (struct cmd), 0) < 0) {
        printf ("确认包发送失败\n") ;
        pthread_exit (0) ;
    }
    printf ("确认信息包发送成功\n") ;
    ...
    char buf[DATAMAX] ;
    int recv_len = 0 ;
    while((recv_len = recv (link_list_ptemp -> sock_fd, buf, sizeof (buf), 0)) > 0) {
        /*传输结束标志*/
        if((recv_len == 4) && (strcmp (buf, "END") == 0)) {
            printf ("buf : %s\n", buf) ;
            memset (buf, 0, sizeof (buf)) ;
            break ;
        }
    ...
    /*向目标用户发送文件*/
    int sendfile_len = 0 ;
    if((sendfile_len = sendfile (file_thid_temp -> sock_fd, fd, NULL, stat_buf.st_size)) < 0) {
    ...
    memset (buf, 0, sizeof (buf)) ;
    strcpy (buf, "END") ;
    if(send (file_thid_temp -> sock_fd, buf, 4, 0) < 0) {
        my_err ("send", __LINE__) ;
        pthread_exit (0) ;
    }
    printf ("sendfile发送了%d字节\n", sendfile_len) ;
    /*等待目标用户断开连接*/
    if((recv_len = recv (file_thid_temp -> sock_fd, buf, sizeof (buf), 0)) < 0) {
        my_err ("recv", __LINE__) ;
        pthread_exit (0) ;
    }
    else if(recv_len == 0) {
        close (file_thid_temp -> sock_fd) ;
    }
    pthread_exit (0) ;
...
/*处理FILEDATA消息包*/
int server_filedata (int sock_fd, struct cmd *cmdtemp)
{
    pthread_t thid ;
    struct file_thid  *file_thid_temp = NULL ;
    /*设置线程参数*/
    file_thid_temp = (struct file_thid *)malloc (sizeof (struct file_thid)) ;
    strcpy (file_thid_temp -> from_ID, cmdtemp -> cmd_from) ;
    strcpy (file_thid_temp -> to_ID, cmdtemp -> cmd_to) ;
    strcpy (file_thid_temp -> filename, cmdtemp -> cmd_data) ;
    file_thid_temp -> sock_fd = sock_fd ;
    /*创建线程转发文件*/
    if(pthread_create (&thid, NULL, (void *)server_filedata_thid, file_thid_temp)) {
...
/*处理FILENAME消息包*/
int server_filename (int sock_fd, struct cmd *cmdtemp)
{
    /*根据目标ID发送FILENAME包*/
    ...
    /*查找源用户*/
    while(online_list_ptemp != NULL) {
        if(strcmp (online_list_ptemp -> user_ID, cmdtemp -> cmd_from) == 0) {
            break ;
        }
        online_list_ptemp = online_list_ptemp -> next ;
    }
    if(online_list_ptemp == NULL) {
        ...
        /*存储套结字*/
        online_list_ptemp -> link_list_head = link_list_add (online_list_ptemp -> link_list_head, link_list_ptemp) ;
        /*从连接表中删去此结点*/
        link_list_PHEAD = link_list_del (link_list_PHEAD, link_list_ptemp) ;
        return 0 ;
    }
...
/****************底层控制函数******************/
/*获取时间字符串*/
void server_time_get (char buf[BUFMAX])
{
    struct tm *t ;
    time_t t_now ;
    time (&t_now) ;
    ...
    /*接收正常的命令包，对其进行解析*/
        ...
        switch(cmdtemp.cmd_flag) {
            ...
            case REGISTER :
                ...
                break ;
            /*客户端传来文件名，表示要发送文件，单独用一个线程接收文件*/
            case FILENAME :
                printf ("i get a FILENAME bag\n") ;
                if(server_filename (poll_fd -> fd, &cmdtemp) < 0) {
                    return -1 ;
                }
                return 1 ;
                break ;
            case FILEDATA :
                printf ("i get a FILEDATA bag\n") ;
                if(server_filedata (poll_fd -> fd, &cmdtemp) < 0) {
                    return -1 ;
                }
                return 1 ;
                break ;
